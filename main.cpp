...
//
// Note: C++ provides a more appropriate std::stack class, except that the task
// requires us to be able to display its contents, and that kind of access is an
// expressly non-stack behavior.
...
Precedence precedence(const Operator_Name& op) {
    return Operators[op].precedence;
}
Associates associativity(const Operator_Name& op) {
...
bool is_parenthesis(const Token& t) {
...
struct Progressive_Display {
    ...
    Progressive_Display& operator()(
        const Token& token)  // Ready the first two columns
    {
        token_name = token;
        token_type =
            is_operator(token)
                ? "op"
                : is_parenthesis(token) ? "()" : is_number(token) ? "num" : "";
        return *this;
    }

    Progressive_Display& operator()(  // Display all columns of a row
        const string& description, const stack<Token>& stack,
        const vector<Token>& output) {
        std::cout << std::right << std::setw(7) << token_name << " │ "
                  << std::left << std::setw(4) << token_type << " │ "
                  << std::setw(16) << description << " │ " << std::setw(12)
                  << (std::ostringstream{} << stack).str() << " │ " << output
                  << "\n";
        return operator()(
            "");  // Reset the first two columns to empty for next iteration
    }
};
...
//-------------------------------------------------------------------------------------------------
{
    ...
    for (auto token :
         tokens)  // Shunting Yard takes a single linear pass through the tokens
        ...
        //.........................................................................
            ...
            if (!is_open_parenthesis(token)) {
                ...
                //   : while prec(token) == prec(top) AND assoc(token) is
                //   left-to-right
                while (
                    !stack.empty() and
